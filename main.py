...
def win():
    score = wn.render('Your score: {}'.format(str(ship.distance)), False, pygame.Color('white'))
    text = wn.render('YOU WIN!', False, pygame.Color('white'))
    screen.blit(fon, (0, 0))
    screen.blit(text, (550, 250))
    screen.blit(score, (530, 290))
...
while not start:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if type(event.ui_element) == Exit_Button:
                sys.exit()
            elif type(event.ui_element) == Restart_Button:
                ship = Ship(screen)
                pause = True
                mouse_pointer = MousePointer(0, 0)
                mouse_traking = False
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.ui_element == tel:
                    ...
                    if event.ui_object_id == '#opening_window.#panel_container.#battery_button':
                        building_module = Battery(ship, 0, 0, building=True)
                    elif event.ui_object_id == '#opening_window.#panel_container.#engine_button':
                        building_module = Engine(ship, 0, 0, building=True)
                    elif event.ui_object_id == '#opening_window.#panel_container.#lab_button':
                        building_module = Lab(ship, 0, 0, building=True)
                    elif event.ui_object_id == '#opening_window.#panel_container.#powerplant_button':
                        building_module = PowerPlant(ship, 0, 0, building=True)
                    elif event.ui_object_id == '#opening_window.#panel_container.#farm_button':
                        building_module = Farm(ship, 0, 0, building=True)
                    elif event.ui_object_id == '#opening_window.#panel_container.#laser_button':
                        building_module = Laser(ship, 0, 0, building=True)
                    elif event.ui_object_id == '#opening_window.#panel_container.#armor_button':
    ...
    while final:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if type(event.ui_element) == Not_Exit_Button:
                    not_exit_window.show()
                elif type(event.ui_element) == Exit_Button:
                    sys.exit()
                elif type(event.ui_element) == Restart_Button:
